  ...
  Widget _buildDownloadList() => ListView(
    ...
    children: _items
        ...
        onItemClick: (YoutubeDownloadModel? task) {
          log(dir.path + '/' + task!.title + '                     bbb           ');
          _openDownloadedFile(task).then((bool success) {

log(dir.path + '/' + task.title);

Navigator.push(context, MaterialPageRoute(builder: (context) => ChewieDemo(title: task.title,)));
  ...
  Future<void> _prepareSaveDir() async {
    ...
    final bool hasExisted = await dir.exists();
    if (!hasExisted) {
      dir.create();
    }
  }
  ...
  Future<String?> _findLocalPath() async {
    ...
    if (Platform.isAndroid) {
      try {
        ...
        String newPath = '';
        final List<String> paths = dir.path.split('/');
